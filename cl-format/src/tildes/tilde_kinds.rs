...
This enum is derived by TildeAble, that proc macro in `cl-format-macro`. Most of the variants inside has its paird trait and methods in `TildeAble` trait those generated by TildeAble marco.

For example, `TildeKind::Digit` will
+ generate trait `TildeKindDigit`
+ and `fn into_tildekind_digit(&self) -> Option<&dyn TildeKindDigit>` method declaration in `TildeAble` trait
+ and auto implement `TildeAble` for several types by adding `fn into_tildekind_digit(&self) -> Option<&dyn TildeKindDigit> {Some(self)}`

That's just implement details. And the auto-impl types are basic types. Check README for custom types.
"]
